...
open class SwiftPhotoGallery: UIViewController {
    ...
    open weak var dataSource: SwiftPhotoGalleryDataSource?
    open weak var delegate: SwiftPhotoGalleryDelegate?

    open lazy var imageCollectionView: UICollectionView = self.setupCollectionView()
    ...
    fileprivate var pageControlCenterXConstraint: NSLayoutConstraint?
    ...
    open func reload(_ imageIndexes:Int...) {
    ...
    #if os(iOS)
    @objc fileprivate func wasDragged(_ gesture: PanDirectionGestureRecognizer) {
    ...
    open func singleTapAction(_ recognizer: UITapGestureRecognizer) {
    ...
    fileprivate func scrollToImage(_ withIndex: Int, animated: Bool = false) {
        imageCollectionView.scrollToItem(at: IndexPath(item: withIndex, section: 0), at: .centeredHorizontally, animated: animated)
    }

    fileprivate func getImage(_ currentPage: Int) -> UIImage {
