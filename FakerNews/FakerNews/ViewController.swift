...
//  FakerNews
//
//  Created by Luke Mann on 1/14/17.
...
class ViewController: UIViewController {
    ...
    var titleDataDisplay:[String]=[]
    var hasRefresfed=false
    ...
    override func viewDidAppear(_ animated: Bool) {
        if !hasRefresfed {
            if isInternetAvailable() {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(446) ) {
                    SwiftOverlays.removeAllBlockingOverlays()
                    self.hasRefresfed=true
                }
    ...
    func getJSON(json: JSON) {
        for (index, object) in json {
            ...
            var notes=object["notes"].stringValue
            if notes==""{
                notes="n/a"
            }
            var credible=false
            var types=object["type"].stringValue
            if types=="credible"{
                credible=true
            }
            if types==""{
            ...
            sources.append(Source(language: language, type: types, notes: notes, name: name, credible: credible)!)
    ...
    func getJSONCredible(json: JSON) {
        for (_,object) in json {
            ...
            var language=object["language"].stringValue
            if language==""{
                language="english"
            }
            sources.append(Source(language: language, type: "n/a", notes: "n/a", name: name, credible: true)!)
            titleData.append(name)
        }
    }
    ...
    func matchFound(source: Source) {
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        appDelegate.name = source.name!
        appDelegate.credible=source.credible!
        appDelegate.type=source.type!
        appDelegate.notes=source.notes!
        appDelegate.language=source.language!
        let viewController:UIViewController = UIStoryboard(name: "DetailView", bundle: nil).instantiateViewController(withIdentifier: "Detail") as UIViewController
    ...
    func setupRAMREELStuff() {
        dataSource = SimplePrefixQueryDataSource(titleDataDisplay)
        ramReel = RAMReel(frame: self.view.bounds, dataSource: dataSource, placeholder: "Start by typing…") {
            print("Plain:", $0)
            print(self.getSourceSingularityFromPrefix(prefix: $0))
            if let source=self.getSourceSingularityFromPrefix(prefix: $0){
                self.matchFound(source: source)
            }
            print("Y01")
            print(self.titleDataDisplay.contains($0))
            print("Yo2")
            if self.titleData.contains($0)||self.titleDataDisplay.contains($0){
                print("HEEEEE")
                for foo in self.sources {
                    if foo.name==$0||foo.altName==$0{
                        self.matchFound(source: foo)
                    }
                }
    ...
    func createAlert() {
        let alertController = UIAlertController(title: "No Connection", message: "An internet connection is required to use this app", preferredStyle: .alert)
    ...
    func getSourceSingularityFromPrefix(prefix:String)->Source?{
        print("In function")
        var array:[Source]=[]
        var count=0
    ...
    func updateLetters() {
        ...
        for i in 0...result.count-1 {
            result[i].truncate(toLength: count, trailing: "...")
        }
    
        titleDataDisplay=result
        print(titleDataDisplay)
    }
   }
...
public extension String {
    ...
    func getAltNames()->String{
        ...
        name.truncate(toLength: count, trailing: "...")
        return name
    }
